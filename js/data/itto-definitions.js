...
export const ittoDefinitions = {
    ...
    'EEFs': { zh: '事业环境因素（Enterprise Environmental Factors，EEFs）是指项目团队不能控制的，将对项目产生影响、限制或指导作用的各种内外部环境条件。包括组织文化、治理结构、基础设施、信息技术软件、资源可用性、员工能力、市场条件、法律法规和行业标准等。', en: 'Enterprise Environmental Factors (EEFs) refer to conditions not under the control of the project team that influence, constrain, or direct the project. They include organizational culture, governance structure, infrastructure, IT software, resource availability, employee capability, market conditions, legal requirements, and industry standards.', category: 'input' },
    'OPAs': { zh: '组织过程资产（Organizational Process Assets，OPAs）是执行组织所特有的并被其使用的计划、流程、政策、程序和知识库。包括正式和非正式的计划、政策、程序、指南、历史信息、经验教训知识库、模板、标准化的指南等。', en: 'Organizational Process Assets (OPAs) are the plans, processes, policies, procedures, and knowledge bases specific to and used by the performing organization. They include formal and informal plans, policies, procedures, guidelines, historical information, lessons learned knowledge bases, templates, and standardized guidelines.', category: 'input' },
    'PMIS': { zh: '项目管理信息系统（Project Management Information System，PMIS）是用于收集、整合和分发项目管理过程输出的自动化工具系统。它支持从项目启动到收尾的整个生命周期，包括进度计划软件、配置管理系统、信息收集与分发系统、线上自动化系统接口等。', en: 'Project Management Information System (PMIS) is an automated tool system used to collect, integrate, and disseminate the outputs of project management processes. It supports the entire project lifecycle from initiation to closure, including scheduling software, configuration management systems, information collection and distribution systems, and online automated system interfaces.', category: 'tool' },
    '设计思维': { zh: '设计思维是一种以人为中心的创新方法论，用于启发和分析需求流程中。它通过同理心、定义、构思、原型和测试五个阶段，深入理解用户需求和问题本质，从而产生创新的解决方案。在敏捷和混合项目中广泛应用。', en: 'Design Thinking is a human-centered innovation methodology used in the Elicit and Analyze Requirements process. Through its five phases—Empathize, Define, Ideate, Prototype, and Test—it deeply understands user needs and problem essence to generate innovative solutions. Widely applied in agile and hybrid projects.', category: 'tool' },
    '滚动波浪规划': { zh: '滚动波浪规划是一种迭代式的规划技术，对近期要完成的工作进行详细规划，而对远期工作则暂时只在较高层级上进行粗略规划。随着项目的推进和信息的逐步明确，远期工作的细节会被逐步细化。特别适用于不确定性高或采用敏捷/混合生命周期的项目。', en: 'Rolling Wave Planning is an iterative planning technique where work to be accomplished in the near term is planned in detail, while work in the future is planned at a higher level. As the project progresses and information becomes clearer, details for future work are progressively elaborated. Particularly useful for projects with high uncertainty or using agile/hybrid lifecycles.', category: 'tool' },
    '整体变更控制': { zh: '整体变更控制是评估与实施变更流程中的核心技术，用于审查所有变更请求、批准变更以及管理变更对可交付成果、项目文件和项目管理计划的修改。它确保只有经过批准的变更才能被纳入修改后的基准中，维护项目基线的完整性。', en: 'Integrated Change Control is a core technique in the Assess and Implement Changes process, used to review all change requests, approve changes, and manage modifications to deliverables, project documents, and the project management plan. It ensures only approved changes are incorporated into revised baselines, maintaining baseline integrity.', category: 'tool' },
    '采购文档': { zh: '采购文档是用于支持采购过程的各种文件的统称，包括采购工作说明书、招标文件（如RFP、RFQ、IFB）、独立成本估算、供方选择标准、合同等。在收尾项目或阶段流程中作为重要的输入资料用于核实采购工作的完成情况。', en: 'Procurement Documents are a collective term for various documents used to support the procurement process, including procurement statement of work, solicitation documents (e.g., RFP, RFQ, IFB), independent cost estimates, source selection criteria, and contracts. They serve as important inputs in the Close Project or Phase process for verifying procurement completion.', category: 'input' },
    '资金提案': { zh: '资金提案是在监督与控制财务流程中产生的输出，当发现项目资金不足或出现额外资金需求时，向组织财务总监和高层发起人提交的正式资金申请文件。它基于挣值分析和完工尚需绩效指数等数据，说明需要额外资金的理由和金额。', en: 'Funding Proposals are outputs from the Monitor and Control Finances process, submitted as formal funding request documents to the organizational financial director and senior sponsor when insufficient funds or additional funding needs are identified. They justify the need and amount of additional funding based on EVM, TCPI, and other financial analysis data.', category: 'output' },
    '状态报告': { zh: '状态报告是在管理相关方参与流程中使用的项目文件，描述项目当前的整体状态，包括已完成的工作、正在进行的工作、下一步计划、关键问题和风险。它为相关方提供项目进展的透明度，是相关方参与和沟通的基础输入之一。', en: 'Status Reports are project documents used in the Manage Stakeholder Engagement process, describing the current overall project status including completed work, work in progress, next steps, key issues, and risks. They provide transparency on project progress to stakeholders and serve as foundational inputs for stakeholder engagement and communication.', category: 'input' },
    '持续改进': { zh: '持续改进是在监督与控制资源利用流程中使用的分析工具，它通过定期评估资源利用的效率，识别可以优化的过程和环节，并系统地实施改进措施。它强调持续的、渐进的改进而非一次性的大变革，常见的方法包括PDCA循环和价值流映射。', en: 'Continuous Improvement is an analysis tool used in the Monitor and Control Resourcing process that regularly evaluates resource efficiency, identifies processes and areas for optimization, and systematically implements improvement measures. It emphasizes ongoing, incremental improvement rather than one-time major changes, with common methods including PDCA cycle and value stream mapping.', category: 'tool' }
};
